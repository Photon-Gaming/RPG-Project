...
namespace RPGGame.GameObject.Entity
{
    ...
    public abstract class InputListenerBase<T>(string name, Vector2 position, Vector2 size, string? texture) : Entity(name, position, size, texture) where T : Enum
    {
        [JsonProperty]
        [EditorModifiable("Input to listen for", "The specific input that this entity instance will listen for")]
        public abstract T ConfiguredInput { get; set; }
    ...
    {
        public override Keys ConfiguredInput { get; set; }
